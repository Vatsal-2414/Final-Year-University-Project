...
!declaration
double precision r(1:3,1:n,-8:8),v(1:3,1:n,-8:8),a(1:3,1:n,-8:8),a_temp(1:3,1:n,0:8),m(1:n),dt,tcurr,tout,G
double precision r_pf(1:3,1:n,-8:8), v_pf(1:3,1:n,-8:8),a_pf(1:3,1:n,-8:8) !r_pf, v_pf, a_pf store the past information and the future positions
double precision dr(1:3), rad2, AU, yr, com(1:3), cov(1:3), mtot, KE, PE, Ei, Ecurr, delE, rad, tcount, dsun(1:n,-8:1)
...
!Time loop created
do p=-7,1                                !keeping track of the steps in time
    ...
    write(6,*) 'A:New x position of Jupiter=',r(1,2,p)
    ...
    do i=1,n
        ...
        do j=1,n
            ...
            do k=1,3
                a(1:3,i,p) = a(1:3,i,p) - (G*m(j)*dr(k)/(sqrt(rad2)*rad2))        !get the future acceleration
            end do
        end do
    end do
    write(6,*) 'B:New y acceleration of Jupiter=',a(2,2,p)
    !future velocity
    do i=1,n
    ...
    if(tcount==techeck) then
        ...
        do i=1,n-1
            do j=i+1,n
                ...
                rad = sqrt(dr(1,p)*dr(1,p) + dr(2,p)*dr(2,p) + dr(3,p)*dr(3,p))
                PE = PE - (G*m(i)*m(j)/rad)
            end do
        end do
